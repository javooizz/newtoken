...
{
    $issuer = app_oidc_issuer_for_prefix(app_oidc_request_prefix());
...
function app_oidc_legacy_prefix()
{
    $prefix = trim((string) app_config('oidc_legacy_prefix', ''));
    if ($prefix === '') {
        return '';
    }
    return '/' . trim($prefix, '/');
...
function app_oidc_request_prefix()
{
    $legacyPrefix = app_oidc_legacy_prefix();
    if ($legacyPrefix === '') {
        return '';
    }
    $path = app_path();
...
function app_oidc_issuer_for_prefix($prefix = '')
{
    $configuredIssuer = rtrim((string) app_config('oidc_issuer', app_base_url()), '/');
    $normalizedPrefix = trim((string) $prefix);
    if ($normalizedPrefix === '') {
        return $configuredIssuer;
    }
    $normalizedPrefix = '/' . trim($normalizedPrefix, '/');
    if (str_ends_with($configuredIssuer, $normalizedPrefix)) {
        return $configuredIssuer;
    }
    return rtrim((string) app_base_url(), '/') . $normalizedPrefix;
}

function app_oidc_path_matches($path, $route)
{
    if ($path === $route) {
        return true;
    }
    ...
    return $path === $legacyPrefix . $route;
}
