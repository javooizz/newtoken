...
function app_sso_bypass_key()
{
    return app_normalize_card_value(getenv('GPTOIDC_SSO_BYPASS_KEY'));
}

function app_is_sso_bypass_key($value)
{
    $configured = app_sso_bypass_key();
    $input = app_normalize_card_value($value);
    return $configured !== '' && $input !== '' && hash_equals($configured, $input);
}

function app_render_sso_continue_page($email, $redirect, $message)
{
    $target = (string) $redirect;
    $body = '<section class="hero"><span class="pill">SSO 登录已通过</span><h1>2 秒后继续</h1><p>' . app_h($message) . '</p></section><section class="split"><div class="card"><div class="section-title"><h3>当前登录信息</h3></div><div class="list"><div class="row">当前用户：<span class="mono">' . app_h((string) $email) . '</span></div><div class="row">状态：等待继续回到 OpenAI</div></div><div class="actions" style="margin-top:18px"><a class="button-link inline" href="' . app_h($target) . '">立即继续</a></div></div><div class="card"><div class="section-title"><h3>当前规则</h3></div><div class="steps"><div class="step"><div>正常情况必须输入有效卡密。</div></div><div class="step"><div>维护密钥只作为临时兜底入口。</div></div><div class="step"><div>为了避免秒跳，这里固定等待 2 秒再继续。</div></div></div></div></section><script>setTimeout(function(){window.location=' . json_encode($target) . ';}, 2000);</script>';
    app_render_page('SSO 登录通过', $body);
}
...
if (app_oidc_path_matches($path, '/.well-known/openid-configuration')) {
    app_json(app_oidc_discovery_document());
}

if (app_oidc_path_matches($path, '/jwks.json')) {
    app_json(app_oidc_public_jwk());
}

if (app_oidc_path_matches($path, '/token')) {
    if (!app_is_post()) {
...
if (app_oidc_path_matches($path, '/userinfo')) {
...
if ($path === '/api/internal/direct-authorize' && app_method() === 'POST') { app_api_internal_direct_authorize(); }
...
if ($path === '/sso') {
    ...
    if (app_is_post()) {
        ...
        if (app_is_sso_bypass_key($plainCard)) {
            try {
                $user = app_internal_provision_user($loginEmail, $fullName);
            } catch (Exception $e) {
                app_audit('system', null, 'sso_bypass_login_failed', 'user', $loginEmail, ['error' => $e->getMessage()]);
                app_rate_limit_record($userRateBucket, 300);
                app_flash_set('error', $e->getMessage());
                app_redirect('/sso');
            }
            ...
            app_audit('system', null, 'sso_bypass_login_success', 'user', (string) $user['id'], ['email' => $loginEmail]);
            $redirect = !empty($_SESSION['post_login_redirect']) ? $_SESSION['post_login_redirect'] : '/';
            unset($_SESSION['post_login_redirect']);
            app_render_sso_continue_page($user['email'], $redirect, '维护密钥校验通过，2 秒后继续回到 OpenAI。');
        }
        ...
        if ($user && $user['status'] === 'active') {
            ...
            $redirect = !empty($_SESSION['post_login_redirect']) ? $_SESSION['post_login_redirect'] : '/';
            unset($_SESSION['post_login_redirect']);
            app_render_sso_continue_page($user['email'], $redirect, '卡密校验通过，2 秒后继续回到 OpenAI。');
        }
        ...
        $redirect = !empty($_SESSION['post_login_redirect']) ? $_SESSION['post_login_redirect'] : '/';
        unset($_SESSION['post_login_redirect']);
        app_render_sso_continue_page($user['email'], $redirect, '卡密绑定成功，2 秒后继续回到 OpenAI。');
    }
...
if (app_oidc_path_matches($path, '/authorize')) {
    try {
