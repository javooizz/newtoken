...
def fetch_remote_account_list(config):
    ...
    while True:
        if page > REMOTE_ACCOUNT_MAX_PAGES:
            raise RuntimeError(
                f"远程账号列表超过 {REMOTE_ACCOUNT_MAX_PAGES} 页，"
                "请检查 Sub2API 分页返回是否异常"
            )
        path = (
...
def delete_dead_remote_accounts(config, dead_items):
    ...
    for item in dead_items:
        try:
            ...
            account_id = 0
        if account_id > 0 and account_id not in unique_items:
            unique_items[account_id] = item
    if not unique_items:
        return {"deleted": 0, "failed": 0, "items": []}
    results = []
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=min(REMOTE_DELETE_CONCURRENCY, len(unique_items))
    ) as executor:
        future_map = {
