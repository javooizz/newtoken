"""OIDC API client for WebUI programmatic access."""
...
def _oidc_request(
    ...
    *,
    ...
    if not api_url or not api_key:
        return {
            "ok": False,
            "error": "OIDC not configured (missing SUB2API_OIDC_API_URL or SUB2API_OIDC_API_KEY)",
        }
    url = f"{api_url}{path}"
    headers = {
...
def oidc_generate_cards(
    ...
    return _oidc_request(
        "POST",
        "/api/cards/generate",
        body={"count": count, "expires_days": expires_days, "note": note},
        config=config,
    )
