...
def record_policy_events(
    ...
    *,
    ...
    reserve_result = result.get("reserve_members") or {}
    for item in reserve_result.get("invited_members") or []:
        store.append(
            ...
            reason="补足储备成员",
            result="success",
            details={"seat_type": item.get("seat_type", "")},
            created_at=timestamp,
        )
    for item in reserve_result.get("skipped_members") or []:
        store.append(
            action="invite_member",
            email=item.get("email", ""),
            account_id=item.get("account_id"),
            reason=item.get("reason", "邀请失败"),
            result="failed",
            details={},
            created_at=timestamp,
        )
