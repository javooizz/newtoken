...
therefore implements the tiny SOCKS5 and HTTP CONNECT subset we need instead of
requiring PySocks or requests.
"""
...
class Socks5ProxyConfig:
    """Parsed outbound proxy definition."""
...
def parse_socks5_proxy_url(proxy_url: str | None) -> Socks5ProxyConfig | None:
    """Parse a supported proxy URL into a proxy config."""
    ...
    if scheme not in {"socks5", "socks5h", "http", "https"}:
        raise ValueError("??????? socks5://?socks5h://?http:// ? https:// ??")
...
def _build_proxy_authorization_header(proxy: Socks5ProxyConfig) -> dict[str, str]:
    """Build HTTP proxy auth headers when username or password is configured."""

    if not proxy.username and not proxy.password:
        return {}
    token = f"{proxy.username}:{proxy.password}".encode("utf-8")
    encoded = base64.b64encode(token).decode("ascii")
    return {"Proxy-Authorization": f"Basic {encoded}"}


class HttpProxyHTTPConnection(http_client.HTTPConnection):
    """HTTPConnection that sends requests through an HTTP proxy."""
    ...
    def putrequest(self, method, url, *args, **kwargs):
        super().putrequest(method, url, *args, **kwargs)
        for key, value in _build_proxy_authorization_header(self._proxy).items():
            self.putheader(key, value)


class HttpProxyHTTPSConnection(http_client.HTTPSConnection):
    """HTTPSConnection that tunnels through an HTTP proxy."""
    ...
    def __init__(self, host: str, port: int | None, *, proxy: Socks5ProxyConfig, timeout=None):
        ...
        self.set_tunnel(
            host,
            port=port,
            headers=_build_proxy_authorization_header(proxy),
        )
...
def _build_connection(
    ...
    *,
    ...
    if proxy.scheme in {"http", "https"}:
        if scheme == "https":
            return HttpProxyHTTPSConnection(host, port=port, proxy=proxy, timeout=timeout)
        return HttpProxyHTTPConnection(host, port=port, proxy=proxy, timeout=timeout)
    if scheme == "https":
