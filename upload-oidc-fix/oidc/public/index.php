...
if ($path === '/admin/settings/save' && app_is_post()) {
    ...
    try {
        if (app_post('action') === 'sync_public_metadata') {
            $result = app_oidc_sync_public_files();
            app_audit('admin', (int) $admin['id'], 'oidc_public_metadata_synced', 'config', 'app', $result);
            app_flash_set('success', '已重新生成公开 OIDC 文件。');
            app_redirect('/admin');
        }
...
if ($path === '/admin') {
    ...
    $body .= '<div class="card"><div class="section-title"><h3>公开文件补救</h3></div><p class="small">如果 OpenAI 不认跳转，只认实际文件，这里会按当前配置重新生成真实的 discovery 与 JWKS 文件。</p><form method="post" action="/admin/settings/save" class="stack"><input type="hidden" name="csrf_token" value="' . app_h(app_issue_csrf_token('admin_settings')) . '"><input type="hidden" name="action" value="sync_public_metadata"><div class="actions"><button class="inline secondary" type="submit">重新生成公开 OIDC 文件</button></div></form></div>';
