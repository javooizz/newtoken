...
这是整个项目的"心脏"。后台调度器周期性地触发一条流水线，无人值守地完成"降级 → 扫描 → 清幽灵席位 → 判服务号水位 → 注册 → 导入 → 发卡"的完整运营闭环。本文逐步拆解每个环节。
...
## 4. run_auto_maintenance 多阶段（`webui/auto.py`）
...
sequenceDiagram
    ...
    Note over AUTO: Phase 2.5 · 清幽灵席位
    AUTO->>SEAT: list_all_users + fetch_remote_account_list(group)
    AUTO->>SEAT: 占 ChatGPT 席位但不在池者 → change_acc_user_seat(Codex)

    Note over AUTO: Phase 3 · 服务号水位判断
    AUTO->>AUTO: n_active = len(active_chatgpt_remote_ids)<br/>need = K - n_active
    alt need ≤ 0 (服务号够)
    ...
    AUTO->>REG: register_batch(need, domain, proxy, workers=1)
...
### 4.5 Phase 2.5 · 清理幽灵 ChatGPT 席位（先降席位）

对齐 main `README` 的"ChatGPT 席位保护：先降席位 → 再检查数量 → 最后补席位"。占着 ChatGPT 席位（`default`）却**不在 Sub2API 服务池**的成员是"幽灵"——白占硬上限 K、不产生服务能力，还会让旧的"按席位数门控"逻辑永久卡死。

```python
pool_emails = fetch_remote_account_list(remote_config, apply_group_filter=True) 的 name/email 集合
for user in seat_core.list_all_users(client):
    if is_chatgpt_seat_type(user.seat_type) and user.email 不在 pool_emails 且 非母号:
        change_acc_user_seat(..., CODEX_SEAT_TYPE)   # 降为 Codex，释放席位
```

幽灵本就不在 active 池，降级**不改变服务号水位**，只是把被占满的硬上限腾出来给真正能服务的新号。降级符合"永不 remove member"——只改席位类型，不踢人。

### 4.6 Phase 3 · 服务号水位判断（三个提前返回）

补号目标不是"Sub2API alive 数"，而是**池内真正在服务的 ChatGPT 号数**——取自 Phase 1 策略结果的 `active_chatgpt_remote_ids`（ChatGPT 席位 ∩ 在池 ∩ 额度足）：

```python
n_active = len(policy_result["active_chatgpt_remote_ids"])  # Phase1 失败则现查 count_chatgpt_seats 兜底
need = max(0, K - n_active)        # K = CHATGPT_SEAT_LIMIT = 2
report["pool_status"] = {active_chatgpt, target=K, ghost_demoted, need}
```
...
| 条件 | 行为 |
|------|------|
| `need <= 0`（`n_active >= K`） | 服务号够，跳过补号（`replenish skipped`） |
| `auto_cfg.enabled == False` | 自动注册关闭，跳过 |
| `email_domain` 为空 | 无注册域名，记 error 并返回 |

> ⚠️ 旧实现用"ChatGPT 席位数 ≥ K"作护栏门控注册——当幽灵席位占满 K 时会**永久卡死不补号**。新实现先清幽灵释放席位、再按服务号水位补号，护栏不再需要。

### 4.7 Phase 4 · 自动注册

```python
register_count = need                                    # K − 池内 active ChatGPT 服务号
...
| 补号目标 | 2 | `CHATGPT_SEAT_LIMIT` | 池内 active ChatGPT 服务号目标数；`need = 目标 − 当前 active`（原 `SUB2API_AUTO_REGISTER_THRESHOLD/COUNT` 已不用于门控，仅 `ENABLED` 生效） |
...
- 任务模型支持同名去重、有界存储、状态机。
- 流水线：降级 → 扫描 → 清幽灵席位 → 判服务号水位 → 注册 → 导入 → 发卡，各阶段独立容错，扫描失败是唯一硬中断点。
